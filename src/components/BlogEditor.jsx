...
function BlogEditor() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://blog-backend-production-7b89.up.railway.app/api/blogs",
          { withCredentials: true }
        );
        const blogs = response.data.message;
        const publishedBlogs = blogs.filter((obj) => obj.status === "published");
  ...
  useEffect(() => {
    debouncedAutoSave.current = debounce(async () => {
      if (!title && !content && !tags) {
        console.log("Please provide all details");
        return;
      }
  ...
  return (
    <div className="p-8 bg-[#121212] h-screen text-white overflow-hidden flex">
      <div className="w-1/4 h-full bg-slate-950">
        <div id="drafts" className="h-1/2 flex flex-col bg-white">
          <h1 className="text-center text-black text-2xl font-bold bg-zinc-800">
            Drafts
          </h1>
          <div className="overflow-auto mt-4 h-full w-full">
            {fetchedDrafts.map((item) => (
              <p
                onClick={() => getBlogById(item)}
                className="h-12 w-full justify-between p-4 cursor-pointer items-center rounded-lg flex font-bold text-orange-950 bg-white hover:bg-slate-200"
                key={item._id}
              >
        ...
        <div id="published" className="h-1/2 flex flex-col bg-white">
          <h1 className="text-center text-black text-2xl font-bold bg-gray-700">
            Published
          </h1>
          <div className="cursor-pointer overflow-auto mt-4 h-full w-full">
            {fetchedBlogs.map((item) => (
              <p
                onClick={() => getBlogById(item)}
                className="h-12 w-full justify-between p-4 items-center rounded-lg flex font-bold text-orange-950 bg-white hover:bg-slate-200"
                key={item._id}
              >
                <span className="overflow-x-hidden text-nowrap w-1/2 cursor-auto">
                  {item.title}
                </span>{" "}
                <span>{item.createdAt.split("T")[0]}</span>
              </p>
      ...
      <form className="w-full h-full relative ml-2" onSubmit={onSubmit}>
        <input
          ...
          className="h-16 w-1/3 p-1 text-black m-1 font-bold rounded-md pl-2"
        />
        <input
          type="text"
          value={tags || ""}
          ...
          className="h-16 w-1/3 text-black p-1 m-1 font-bold rounded-md pl-2"
        />
        <ReactQuill
          theme="snow"
          value={content || ""}
